...
public class AzureManager {
	...
	private final Pattern patternIpAddress = Pattern
	...
	/**
	 * Create a new connection object based on the provided parameters and stores it
	 * in a cache that is keyed-off of the given account name. If that cache already
	 * holds an existing connection config for that account name then it will be
	 * overwritten / updated
	 */
	public boolean createConnection(String appId, String tenantId, String key, AzureEnvironment azureEnvironment,
			String subscription) {
		...
		ApplicationTokenCredentials credentials = new ApplicationTokenCredentials(appId, tenantId, key,
		...
		if (enableDebugRestClient) {
			restClient = new RestClient.Builder(httpClientBuilder, retrofitBuilder).withCredentials(credentials)
	...
	/**
	 * Initializes the global member awsClientConfig. This is required before any
	 * interaction with the AWS service! The method tries to load the custom Java
	 * trustStore that holds the Amazon Root CA certificate. This trustStore is used
	 * to build a custom SSLContext which is then loaded into the custom client
	 * config. This is then used in all calls to the AWS library that interact with
	 * the AWS cloud to ensure they trust any Amazon server certificates.
	 * 
	 * @param socketTimeout     Timeout in milliseconds to wait for the socket to
	 *                          any AWS API server to setup
	 * @param connectionTimeout Timeout in milliseconds to wait for a result on any
	 *                          API request towards any AWS server
	...
	private OkHttpClient.Builder buildAzureRestClient(int socketTimeout, int connectionTimeout) {
		...
		try {
			/*
			 * Load the keyStore file. It is located within the WAR file under
			 * /WEB-INF/classes which is a default folder for the classloader to search for
			 * files to load. Within the project structure, the file is located under
			 * src/main/resources
			 */
			...
			// String awsKeyStorePath = System.getProperty("jboss.server.config.dir") +
			// "/connect/awsJavaKeyStore";
			// InputStream awskeyStoreStream = new FileInputStream(awsKeyStorePath);

			if (azureKeyStoreStream == null) {
				logger.error(
						"Couldn't find custom Azure Java trust store file: azureJavaKeyStore - won't be able to communicate with the Azure cloud!");
				return null;
			}

			// Create a custom keystore and load the content of the azureJavaKeyStore into
			// it
			...
			if (trustManager.getAcceptedIssuers() != null && trustManager.getAcceptedIssuers().length > 0) {
				...
				for (X509Certificate issuer : trustManager.getAcceptedIssuers()) {
					logger.debug("TrustManager accepted issuer: DN: " + issuer.getIssuerDN() + ", subject DN: "
							+ issuer.getSubjectDN());
				}
	...
	/**
	 * Returns the list of all existing "managed" security groups from the given
	 * Azure account. Uses a special tag ("ExtremePolicyId") as a filter on each
	 * security group to limit the result to only those groups that are synchronized
	 * by Connect.
	 * 
	 * @param resourceGroupName Optional. If provided, the method will return only
	 *                          managed security groups from the given resource
	 *                          group
	 */
	public HashMap<String, NetworkSecurityGroup> retrieveManagedSecurityGroups(String accountName,
			String resourceGroupName) {

		Azure azureConnection = azureConnections.get(accountName);
		if (azureConnection == null) {
			logger.warn("Cannot retrieve the managed security groups since there is no Azure connection for account "
					+ accountName);
			return null;
		}

		logger.debug("Trying to retrieve managed security groups from Azure account " + accountName); // + " //
																										// awsConnection.getRegionsToIgnore());
				...
				if (secGroup.tags().keySet().contains("ExtremePolicyId")) {
					...
					if (resourceGroupName != null && !resourceGroupName.isEmpty()) {
						...
						if (secGroup.resourceGroupName().equalsIgnoreCase(resourceGroupName)) {
							logger.debug("Retrieved next managed sec group that is also part of resource group "
									+ resourceGroupName + ": " + jsonMapper.writeValueAsString(secGroup));
							secGroups.put(secGroup.id(), secGroup);
						} else {
							logger.debug("Retrieved next managed sec group " + secGroup.name()
									+ " but it is not part of resource group " + resourceGroupName + " ("
									+ secGroup.resourceGroupName() + ") - ignoring");
						}
	...
	/**
	 * Returns the list of all existing security groups from the given Azure
	 * account.
	 * 
	 * @param resourceGroupName Optional. If provided, the method will return only
	 *                          managed security groups from the given resource
	...
	public List<Object> retrieveSecurityGroups(String accountName, String resourceGroupName) {
		...
		try {
			...
			// Filter groups based on given resource group?
			if (resourceGroupName != null && !resourceGroupName.isEmpty()) {
				...
				for (NetworkSecurityGroup secGroup : securityGroups) {
					...
					if (secGroup.resourceGroupName().equalsIgnoreCase(resourceGroupName)) {
						...
						logger.debug(
								"Retrieved next sec group " + secGroup.name() + " but it is not part of resource group "
										+ resourceGroupName + " (" + secGroup.resourceGroupName() + ") - ignoring");
					}
				}

				logger.debug("Found a total of " + countTotalGroups + " security groups from account " + accountName
						+ " out of which " + secGroups.size()
						+ " groups got imported because they are within resource group " + resourceGroupName);

			} else {
				// Add all groups - no filter
				secGroups.addAll(securityGroups);
				logger.debug("Imported all " + secGroups.size() + " securtiy groups from Azure");
			}
	 ...
	 * 
	 * @param groupId The id of the security group to retrieve. Will be used as the
	 *                only filter for the request
	 * @return The Azure security group matching the provided security group id or
	 *         null
	 */
	public NetworkSecurityGroup retrieveSecurityGroup(String accountName, String groupId) {
		...
		Azure azureConnection = azureConnections.get(accountName);
		if (azureConnection == null) {
			logger.warn("Cannot retrieve the security group with id " + groupId
					+ " since there is no Azure connection for account " + accountName);
			return null;
		}
		...
		try {
			...
			logger.error("Error retrieving security group with id " + groupId + " from account " + accountName + ": ",
					ex);
			return null;
		}
	}
	 ...
	 * 
	 * @return The full list of all VMs. HashMap key: VM ID in lower-case! Azure
	 *         provides the VM id in slightly different cases depending on where it
	 *         is provided: When retrieving a VM, the resource group part of the id
	 *         is upper-case. But the vm id reference within a network interface
	 *         uses a lower-case resource group. To quickly find the corresponding
	 *         VM within the cache all code should use the lower-case version of the
	 *         id.
	 */
	public List<Object> retrieveVMs(String accountName) {
		...
		if (azureConnection == null) {
			logger.warn(
					"Cannot retrieve virtual machines since there is no Azure connection for account " + accountName);
			return null;
		}
		...
		try {
			...
			logger.debug("Successfully retrieved " + allInstances.size() + " VMs from account " + accountName);
			return allInstances;
	...
	public List<Object> retrieveNetworkInterfaces(String accountName) {
		...
		if (azureConnection == null) {
			logger.warn(
					"Cannot retrieve network interfaces since there is no Azure connection for account " + accountName);
			return null;
		}
		...
		try {
			...
			allNICs.addAll(nicList);
			// for (NetworkInterface nic : nicList) {
			// allNICs.put(nic.id(), nic);
			// }

			logger.debug(
					"Successfully retrieved " + allNICs.size() + " network interfaces from account " + accountName);
			return allNICs;
	...
	/**
	 * Retrieves all networks from the given account. The HashMap's key is the
	 * network id in lower-case
	 */
	...
	/**
	 * Assigns the given security group to the given instance interface. Azure does
	 * not support assigning multiple security groups to an interface.
	 * 
	 * @param instanceId The instance interface to modify the security group for
	 * @param secGroup   The security group to assign to the instance interface. If
	 *                   null is provided, any associated security group will be
	 *                   removed from that interface
	 */
	public boolean assignSecurityGroupToNwInterface(String accountName, NetworkInterface nwIf,
		...
		if (nwIf == null) {
			logger.warn(
					"Cannot assign a security group to a network interface since there is no network interface provided for "
							+ accountName);
			return false;
		}
		...
		if (azureConnection == null) {
			logger.warn(
					"Cannot assign a security group to a network interface since there is no Azure connection for account "
							+ accountName);
			return false;
		}
		...
		try {
			...
			if (secGroup == null) {
				logger.info(
						"Removing security group from VM " + nwIf.virtualMachineId() + " and interface " + nwIf.id());
				updatedNwIf = nwIf.update().withoutNetworkSecurityGroup().apply();
			} else {
				logger.info("Assigning security group " + secGroup.name() + " to VM " + nwIf.virtualMachineId()
						+ " on interface " + nwIf.id());
